...
def create_problem_detailedly(problem_id: int, db: sqlite3.Connection = Depends(get_db)):
    ...
    # 1. Truy vấn lấy thông tin bài tập kèm số lượng testcases cấu hình của Roadmap liên kết
    cursor.execute("""
        SELECT p.name, r.repository_url, r.name AS roadmap_name, r.num_test_cases
        FROM problems p
    ...
    num_test_cases = record["num_test_cases"] or 3  # Đọc số lượng testcase cấu hình động [4]
    name_slug = normalize_problem_name(name)
    problem_folder = f"{storage_path}problems/{name_slug}"
    
    # 2. Tạo prompt truyền động cấu hình số lượng testcase cho AI biên soạn
    try:
    ...
    # 3. Gửi yêu cầu phân tích sâu tới AI DeepWiki
    ...
    try:
        # Phân tích cú pháp và tự sửa lỗi JSON
        ...
        # Định nghĩa đường dẫn tệp tin (.md cho tài liệu, .py cho code)
        statement_path = os.path.join(problem_folder, "statement.md")
        theory_path = os.path.join(problem_folder, "theory.md")
        tutorial_path = os.path.join(problem_folder, "tutorial.md")
        solution_path = os.path.join(problem_folder, "solution.py")  # Đồng bộ sang .py
        coding_path = os.path.join(problem_folder, "coding.py")      # Đồng bộ sang .py
        checker_path = os.path.join(problem_folder, "checker.py")    # Lưu trữ checker.py
        ...
        # Sử dụng trích xuất mã nguồn Python sạch từ markdown code block của AI
        with open(solution_path, "w", encoding="utf-8") as f:
        ...
        temp_input_dir = os.path.join(problem_folder, "inputs")
        temp_output_dir = os.path.join(problem_folder, "outputs")
        os.makedirs(temp_input_dir, exist_ok=True)
        os.makedirs(temp_output_dir, exist_ok=True)

        # Giải mã chuỗi JSON chứa mảng inputs do AI sinh ra
        try:
            test_inputs_list = json.loads(materials.test_inputs)
        except Exception:
            test_inputs_list = []

        # Đảm bảo danh sách khớp chính xác số lượng yêu cầu cấu hình
        if len(test_inputs_list) < num_test_cases:
            while len(test_inputs_list) < num_test_cases:
                # Điền dự phòng một bộ cơ bản nếu AI sinh thiếu
                test_inputs_list.append(test_inputs_list[-1] if test_inputs_list else {})

        # Tiến hành lưu tệp tin input và chạy solution.py để sinh tệp output động tương thích tương ứng [4]
        for i in range(1, num_test_cases + 1):
            ...
            temp_input_json = os.path.join(problem_folder, "input.json")
            temp_output_json = os.path.join(problem_folder, "output.json")
            ...
            try:
                ...
                if process.returncode == 0 and os.path.exists(temp_output_json):
                    ...
                    print(f"[ERROR] Generator failed for testcase {i}: {process.stderr.strip()}")
                    with open(out_file_path, "w", encoding="utf-8") as f_out:
                        json.dump({"error": f"Execution failed: {process.stderr.strip()[:100]}"}, f_out)
            except Exception as e:
        ...
        # Đóng gói ZIP nén lưu trữ để tải về
        input_zip_path = os.path.join(problem_folder, "input.zip")
        output_zip_path = os.path.join(problem_folder, "output.zip")
        ...
        # 5. CẬP NHẬT DATABASE ĐỒNG BỘ: Ghi nhận đường dẫn thư mục test thô (cho run/submit) và tệp zip [1]
        cursor.execute("""
            UPDATE problems 
            SET theory_path = ?, tutorial_path = ?, solution_path = ?, coding_path = ?, checker_path = ?,
                input_folder_path = ?, output_folder_path = ?, input_zip_path = ?, output_zip_path = ?
            WHERE id = ?
        """, (
            theory_path, 
            tutorial_path, 
            ...
            coding_path, 
            checker_path,
            temp_input_dir,   # Cập nhật thư mục testcase thô giúp /run hoạt động được [1]
            temp_output_dir,  # Cập nhật thư mục testcase thô giúp /submit hoạt động được [1]
            input_zip_path, 
            output_zip_path, 
            problem_id
        ))
        ...
        return {
            "status": "success",
            "message": f"Successfully generated educational materials and {num_test_cases} custom test cases.",
            "data": {
                "problem_id": problem_id,
                "name": name,
                "statement_path": statement_path,
                ...
                "coding_path": coding_path,
                "checker_path": checker_path,
                "input_folder_path": temp_input_dir,
                "output_folder_path": temp_output_dir
            }
        }
        ...
        raise HTTPException(status_code=502, detail="AI response JSON format is invalid.")
    except sqlite3.Error as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Database error during material generation: {str(e)}")
    except IOError as e:
